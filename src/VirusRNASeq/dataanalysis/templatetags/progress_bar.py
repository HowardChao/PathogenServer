...
from dataanalysis.views import upload_progress
...
def progress_bar():
    ...
    #     'hidden></progress>' % (uuid.uuid4(), uuid.uuid4())
    progress_bar_tag = '<div class = "progress" hidden>' + '<div class="progress-bar progress-bar-striped progress-bar-animated "' + \
        'role="progressbar" aria-valuenow = "0" data-progress_bar_uuid=' + \
        str(uuid.uuid4()) + \
...
def progress_bar_media():
    ...
    if include_jquery:
        js = ["//code.jquery.com/jquery-3.3.1.js"]
    else:
        js = []
